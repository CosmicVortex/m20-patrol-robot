...
| 功能 | 状态 | 实现文件 |
|------|------|----------|
| APDU/ASDU 编解码 | ✅ implemented | `protocol/frame.py`, `protocol/messages.py` |
| 状态消息解析 | ✅ implemented | `robot/status.py` |
| TCP 客户端 + 门禁 | ✅ implemented | `robot/basic_client.py` |
| 实时状态订阅 | offline_verified | `robot/telemetry.py` |
| 导航报文构造 | implemented | `navigation/protocol.py` |
| Web 服务入口 | offline_verified | `server.py` |
| 认证模块 | implemented | `auth/store.py`, `auth/middleware.py` |
| 云台控制 | implemented | `gimbal/adapter.py`, `api/extended_handlers.py` |
| 视频流管理 | implemented | `video/stream_manager.py` |
