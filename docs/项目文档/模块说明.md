...
│   └── v010.py        # 导航报文构造
├── gimbal/            # 云台控制
│   └── adapter.py     # 数尔云台适配器；HTTP处理器位于api/extended_handlers.py
├── video/             # 视频流
...
client.send_control(msg)         # 控制命令；当前部署链路未建立控制连接
```

门禁条件：`control_enabled=True` + 三份证据（协议版本、固件版本、授权记录）。当前只读部署不满足控制连接条件，状态为 `blocked`。
...
**传输模式**：
- `read_only=False` 时允许订阅返回控制相关字段（如 HES 状态）
- 控制命令的发送由 `BasicServerClient.control_enabled` + Web 授权双重门禁保护
- 控制操作仅通过 Web UI 触发，底层 API 受认证和角色校验保护
...
- 云台可见光：`rtsp://10.21.31.108:554/id=1&type=0`（从manifest读取）
- 云台热成像：`rtsp://10.21.31.108:554/id=2&type=0`（从manifest读取）
...
| `navigation/v010.py` | `test_navigation_v010.py`, `test_navigation_service.py` |
| `gimbal/adapter.py` | `test_gimbal_adapter.py` |
| `auth/middleware.py` | `test_auth_middleware.py`, `test_auth_store.py` |
| `video/stream_manager.py` | `test_video_stream_manager.py`, `test_video_stream_config.py` |
| `motion/service.py` | `test_motion_service.py` |
| `api/handlers.py` | `test_api_response.py`, `test_api_router.py` |
| 集成测试 | `test_frontend_contract.py`, `test_web_runtime_integration.py` |

共23个测试文件，覆盖核心协议、遥测、导航、运动控制、云台、视频流、认证鉴权等模块。
