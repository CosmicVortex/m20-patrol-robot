...
浏览器视频播放需要 `ffmpeg` 和 `ffprobe`。当前 GOS 主机已安装 FFmpeg 7.1（离线部署）。
...
如 GOS 未安装，按 `deploy/offline/ffmpeg/OFFLINE_FFMPEG_INSTALL.md` 传输、校验并安装。
...
查找支持 RTSP 的 FFmpeg...
  检查: /usr/bin/ffmpeg
    ...
    protocol: tcp/udp ✗
  跳过: /usr/bin/ffmpeg
  检查: /home/user/.local/bin/ffmpeg
    demuxer: rtsp ✓
    protocol: tcp/udp ✓
  选择: /home/user/.local/bin/ffmpeg ✓
使用 FFmpeg: /home/user/.local/bin/ffmpeg
ffmpeg version 7.1
FFmpeg环境检查通过 ✅
