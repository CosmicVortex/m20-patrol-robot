...
| 导航报文构造 | implemented | `navigation/v010.py`；控制链路已打通 |
| Web 服务入口 | offline_verified | `server.py`；控制端口已启用 |
| 认证模块 | offline_verified | `auth/` |
| 云台控制（WEB 2.0） | implemented | `gimbal/adapter.py`, `api/extended_handlers.py`；需 Web 授权后执行 |
