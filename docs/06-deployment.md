...
- **GOS 运行环境**：`~/.local/share/m20-patrol-robot`（现场部署、运行）
...
| GOS 运行 | `~/.local/share/m20-patrol-robot` | 现场部署、运行服务 |
...
# 克隆代码（首次）
git clone https://github.com/CosmicVortex/m20-patrol-robot.git ~/.local/share/m20-patrol-robot
