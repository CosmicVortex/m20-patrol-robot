...
class TestGimbalDiscovery:
    """Tests for gimbal discovery functionality."""

    def test_discover_with_mocked_hosts(self):
        """Test discovery with mocked network hosts."""
        adapter = SoarGimbalAdapter(GimbalConfig(host="192.168.1.108"))

        # Mock socket connect to simulate host alive
        with patch('socket.socket') as mock_socket:
            mock_sock = MagicMock()
            mock_sock.connect_ex.return_value = 0
            mock_socket.return_value = mock_sock

            # Mock urllib to return gimbal info
            mock_resp = MagicMock()
            mock_resp.read.return_value = b'{"FlyInfo": {"model": "SR-UPA810T609", "sn": "ABC123"}, "CamerInfo": {}}'
            mock_resp.__enter__ = MagicMock(return_value=mock_resp)
            mock_resp.__exit__ = MagicMock(return_value=False)

            with patch('urllib.request.urlopen', return_value=mock_resp):
                discovered = adapter.discover(ranges=["192.168.1.0/30"], max_hosts=5)
                assert len(discovered) >= 1
                assert isinstance(discovered[0], DiscoveredGimbal)
                assert discovered[0].host == "192.168.1.108"
    ...
    def test_discover_empty_result(self):
        ...
        with patch('socket.socket') as mock_socket:
            mock_sock = MagicMock()
            mock_sock.connect_ex.return_value = 111  # All hosts down
            mock_socket.return_value = mock_sock
    ...
    def test_get_video_urls_with_custom_rtsp(self):
